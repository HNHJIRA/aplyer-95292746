...
export function AbDemo({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col px-6 pt-3">
      <div className="inline-flex items-center gap-1.5 self-start rounded-full border border-brand-green/25 bg-brand-green/10 px-2.5 py-1 font-mono text-[11px] uppercase tracking-[0.18em] text-brand-green">
      ...
      <div className="popup-scroll -mx-6 mt-3 flex-1 space-y-3 overflow-y-auto px-6">
        ...
        {loading && (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-paper p-3 text-[14px] text-muted-foreground">
            <Loader2 className="h-4 w-4 animate-spin" /> Writing in your voice…
          </div>
        )}

        {!loading && error && (
          <div className="rounded-xl border border-[#E5B73A]/40 bg-[#E5B73A]/10 p-3 text-[14px]">
          ...
          <>
            {reveal ? (
              ...
              <button
                ...
                className="w-full rounded-xl border border-dashed border-brand-green/40 bg-brand-green/5 p-3 text-[14px] font-semibold text-brand-green"
              >
                Reveal your version →
              </button>
            )}
...
function Card({ label, text, tone }: { label: string; text: string; tone: "muted" | "brand" }) {
  ...
  return (
    <div className={`rounded-xl border ${border} ${bg} p-3`}>
      <div className={`font-mono text-[11px] uppercase tracking-[0.16em] ${chip}`}>{label}</div>
