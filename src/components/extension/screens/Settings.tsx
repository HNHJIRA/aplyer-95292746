      ...
      <div className="popup-scroll flex gap-0 overflow-hidden">
        <nav className="popup-scroll w-[112px] flex-shrink-0 overflow-y-auto border-r border-border bg-paper/40 py-2">
          {SECTIONS.map((s) => (
            <button
              ...
              className={`flex w-full items-center gap-2 px-3 py-2 text-left text-[12px] transition ${section === s.id ? "border-l-2 border-brand-green bg-brand-green/10 text-brand-green" : "border-l-2 border-transparent text-muted-foreground hover:bg-field hover:text-foreground"}`}
            >
              {s.icon}
              {s.label}
            </button>
        ...
        <div className="popup-scroll flex-1 overflow-y-auto p-4">
          <motion.div key={section} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
            {section === "account" && (
              <Card title="Account">
                ...
                <button
                  ...
                  className="mt-1.5 flex w-full items-center justify-center gap-1.5 rounded-md border border-brand-red/30 bg-brand-red/10 px-3 py-2 text-[13px] font-semibold text-brand-red transition hover:bg-brand-red/15"
                >
                  ...
                  <div key={p.id} className={`rounded-xl border p-3 ${p.highlighted ? "border-brand-green/40 bg-brand-green/5" : "border-border bg-paper"}`}>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[12px] text-muted-foreground">{p.price}</p>
                      </div>
                      {state.subscriptionStatus.tier === p.id ? (
                        <span className="rounded-full bg-brand-green/15 px-2 py-0.5 font-mono text-[10px] uppercase text-brand-green">Current</span>
                      ) : (
                    ...
                    <ul className="mt-2 space-y-1">
                      {p.features.map((f) => (
                        <li key={f} className="flex items-center gap-1.5 text-[12px] text-sub">
            ...
            {section === "ai" && (
              <div className="space-y-2.5">
                <p className="text-[12px] text-muted-foreground">Choose the AI model that powers your answers. Activated in a future milestone.</p>
                {PROVIDERS.map((p) => {
            ...
            {section === "privacy" && (
              <Card title="Privacy">
                ...
                <p className="mt-3 text-[12px] text-muted-foreground">Aplyer keeps your resume and profile on this device. Nothing is sent without your explicit action.</p>
              </Card>
            )}
          </motion.div>
        </div>
          ...
          >
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-[16px] font-bold">Sign out?</h3>
              </div>
              ...
              <div className="mt-4 flex gap-2">
                ...
                <button
                  ...
                  className="flex flex-1 items-center justify-center gap-1.5 rounded-md bg-brand-red px-3 py-2 text-[13px] font-semibold text-white transition hover:bg-brand-red/90 disabled:opacity-60"
                >
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 border-b border-border/60 py-1.5 last:border-0">
      <span className="text-[12px] capitalize text-muted-foreground">{label}</span>
...
function LinkRow({ label }: { label: string }) {
  return (
    <button className="flex w-full items-center justify-between rounded-md px-2 py-2 text-left text-[13px] text-foreground hover:bg-field">
      {label}
