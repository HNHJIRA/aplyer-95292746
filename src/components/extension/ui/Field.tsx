...
export const Field = forwardRef<HTMLInputElement, Props>(function Field(
  ...
  return (
    <div className="flex flex-col gap-1.5">
      {label && (
        <label htmlFor={inputId} className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">
          {label}
        </label>
      )}
      <input
        ...
        id={inputId}
        className={cn(
          "h-10 w-full rounded-lg border border-border bg-field px-3 text-[13px] text-foreground placeholder:text-dim",
      ...
      />
      {error ? (
        <span className="text-[11px] text-brand-red">{error}</span>
      ) : hint ? (
